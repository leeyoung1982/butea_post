...
享寫 是一个**开源的沉浸式中文写作编辑器**，原生支持多平台发布：你在一个全屏所见即所得编辑器里完成写作和排版，享寫 帮你把它**重新表达**成每个目标平台最自然的形态。
...
享寫 是**纯前端**应用 —— 没有服务器,没有数据库,没有用户系统。你的内容、key、草稿都在浏览器里（IndexedDB + localStorage）。
